...
def send_recommendations(recommendation_module, config_path=None):
    ...
    discord_module.send_recommendations()import logging
import requests
import json
import configparser
import os
...
class DiscordModule:
    ...
    def send_recommendations(self):
        """Send recommendations to output (console or Discord)"""
        # Print performer recommendations
        performer_recs = self.format_performer_recommendations()
        print("\n" + performer_recs)
        
        # Print scene recommendations
        scene_recs = self.format_scene_recommendations()
        print("\n" + scene_recs)
        ...
        if self.webhook_url:
            try:
                self._send_to_discord(performer_recs + "\n" + scene_recs)
            except Exception as e:
                logger.error(f"Failed to send recommendations to Discord: {e}")
